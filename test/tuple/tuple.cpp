...
TEST(TestCase, rtuple_test) {
	...
	rtuple<int, const int&, int> rtuple2(d, e, std::ref(d));
	// std::tuple<int, const int&, int&> tuple2(d, e, std::ref(d));

	EXPECT_EQ(std::get<0>(rtuple2), d);
	EXPECT_EQ(std::get<1>(rtuple2), e);
	EXPECT_EQ(std::get<2>(rtuple2), d);
...
TEST(TestCase, mtuple_test) {
	...
	mtuple<int, const int&, int> mtuple2(d, e, std::ref(d));
	// std::tuple<int, const int&, int&> tuple2(d, e, std::ref(d));

	EXPECT_EQ(std::get<0>(mtuple2), d);
	EXPECT_EQ(std::get<1>(mtuple2), e);
	EXPECT_EQ(std::get<2>(mtuple2), d);
